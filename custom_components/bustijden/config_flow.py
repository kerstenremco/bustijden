...
class BustijdenConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Bustijden config flow."""
    ...
    async def _get_stops(self, query: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{API_URL}/stops?q={query}", ssl=False) as response:
